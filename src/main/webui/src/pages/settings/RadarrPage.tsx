...
import type { RadarrServer, JobProgressEvent } from "../../api/types";
import { JobProgressBar } from "../../components/JobProgressBar";
import { useApi } from "../../hooks/useApi";
import { useJobProgress } from "../../hooks/useJobProgress";

export default function RadarrPage() {
  const { data: servers, error: loadError, reload } = useApi(() => api.listRadarrServers(), []);
    ...
    <div>
      <div className="page-header" style={{ padding: "0 0 4px" }}>
        <div style={{ flex: 1, minWidth: 280 }}>
          <h2 style={{ marginBottom: 6 }}>Radarr</h2>
          <p className="text-muted" style={{ maxWidth: "60ch" }}>
            Migrating from Radarr? Connect it here to import your existing movie library — every
            movie with a file already on disk lands in Kosmos, matched by TMDB id. Movies Radarr
            has monitored but not yet downloaded aren't imported.
          </p>
        </div>
      ...
      {loadError && <p className="text-muted">Failed to load Radarr servers: {loadError}</p>}
      {servers?.length === 0 && <p className="text-muted">No Radarr server connected yet.</p>}

      {servers?.map((server) => (
        <RadarrServerRow key={server.id} server={server} showToast={showToast} />
      ))}
      ...
      {modalOpen && (
        <AddRadarrServerModal
          ...
          onCreated={() => {
            setModalOpen(false);
            reload();
            showToast("Radarr server added — sync its library below.");
          }}
        />
      )}
...
function RadarrServerRow({
  ...
  const jobName = `radarr-library-sync-${server.id}`;
  ...
  async function runSync() {
    ...
    setClicking(true);
    try {
      const run = await api.syncRadarrLibrary(server.id);
      showToast(
        run.status === "FAILED"
  ...
  async function registerRootFolders() {
    ...
    setRegistering(true);
    try {
      const result = await api.autoRegisterRootFoldersFromRadarr(server.id);
...
function AddRadarrServerModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  async function testConnection() {
    ...
    setTesting(true);
    try {
      const result = await api.testRadarrConnection({ baseUrl: baseUrl.trim(), apiKey: apiKey.trim() });
  ...
  async function save() {
    ...
    setError(null);
    try {
      await api.createRadarrServer({ name, baseUrl, apiKey });
      onCreated();
    } catch (e) {
      ...
      <div className="dialog" onClick={(e) => e.stopPropagation()}>
        <div className="dialog-header">
          ...
          <div className="dialog-header-body">
            <div className="dialog-title">Add Radarr server</div>
            <div className="dialog-sub">Create an API key under Settings → General in Radarr.</div>
          </div>
        ...
        <div className="field">
          <label>Name</label>
          <input className="input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Radarr" />
        </div>
